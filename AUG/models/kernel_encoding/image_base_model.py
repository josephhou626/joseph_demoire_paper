...
class ImageBaseModel(BaseModel):
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            self.optimizer_G = torch.optim.Adam(
                optim_params, lr=train_opt["lr_G"], weight_decay=wd_G, betas=(
                    train_opt["beta1"], train_opt["beta2"])
            )
    ...
    def cal_params(self):
        print('Parameter: %d' % sum(p.numel() for p in self.netG.parameters() if p.requires_grad))
    ...
    def get_current_visuals(self, need_GT=True):
        ...
        out_dict["fake_LQ"] = self.fake_LQ.detach()[0].float().cpu()
        return out_dict
