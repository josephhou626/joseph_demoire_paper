...
class KernelExtractor(nn.Module):
    def __init__(self, opt):
        ...
        output_nc = (self.kernel_dim * 1) if self.use_vae else self.kernel_dim
        ...
        for i in range(n_downsampling):  # add downsampling layers
            ...
            model += [
                nn.Conv2d(inc, ouc, kernel_size=3, stride=2,
...
class KernelWizard(nn.Module):
    def __init__(self, opt):
        ...
        feature_extractor.append(
            nn.Conv2d(num_image_channels, nf, 3, 1, 1, bias=True))
        ...
        recon_trunk.append(lrelu)
        recon_trunk.append(
            nn.Conv2d(64, num_image_channels, 3, 1, 1, bias=True))
