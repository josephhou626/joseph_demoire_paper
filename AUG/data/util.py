...
def _get_paths_from_lmdb(dataroot):
    ...
    # print(os.path.join(dataroot, "meta_info.pkl"))
    # meta_info = pickle.load(
    #     open(os.path.join(dataroot, "meta_info.pkl"), "rb"))
    meta_info = pickle.load(
        open((dataroot+"/meta_info.pkl"), "rb"))
...
def rgb2ycbcr(img, only_y=True):
    ...
    if only_y:
        ...
        rlt = np.matmul(
            img, [[65.481, -37.797, 112.0], [128.553, -
                                             74.203, -93.786], [24.966, 112.0, -18.214]]
        ) / 255.0 + [16, 128, 128]
    if in_img_type == np.uint8:
        rlt = rlt.round()
    else:
...
def bgr2ycbcr(img, only_y=True):
    ...
    if only_y:
        ...
        rlt = np.matmul(
            img, [[24.966, 112.0, -18.214], [128.553, -
                                             74.203, -93.786], [65.481, -37.797, 112.0]]
        ) / 255.0 + [16, 128, 128]
    if in_img_type == np.uint8:
        rlt = rlt.round()
    else:
...
def ycbcr2rgb(img):
    ...
    rlt = np.matmul(
        img, [[0.00456621, 0.00456621, 0.00456621], [
            0, -0.00153632, 0.00791071], [0.00625893, -0.00318811, 0]]
